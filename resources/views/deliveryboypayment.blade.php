...
@section('page-content')
    ...
    <section class="content-header">
        <h1>
        <b>Delivery Boy Payment</b>
        </h1>
        <ol class="breadcrumb">
            <li><a href="/dashboard"><i class="fa fa-dashboard"></i>Home
            </a></li>
            <li class="active">Delivery Boy Payment</li>
        </ol>
    </section>
                            ...
                            <div class="table-responsive">
                                <table  id="example" class="table table-bordered table-hover retailer-search">
                                    ...
                                    @foreach($retailer as $item)
                                        ...
                                        <td><i class="fa fa-inr">{{$item->payable_amount}}</i></td>
                                            <td><i class="fa fa-inr">{{$item->paid_amount}}</i></td>
                                        <td>{{$item->user_name}}</td>
                                        <td>{{$item->user_mob}}</td>
                                        <td><a href="{{URL::to('paymentdboyupdate/'.$item->id) }}">
